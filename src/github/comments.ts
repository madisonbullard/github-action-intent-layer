...
/**
 * Hidden marker for intent layer link comments.
 * This marker identifies comments that link to an intent layer PR.
 */
export const INTENT_LAYER_LINK_MARKER = "<!-- INTENT_LAYER_LINK -->";

/**
 * Generate a comment body that links to the intent layer PR.
 *
 * This comment is posted on the original PR when `output: new_pr` mode
 * creates a separate PR for intent layer updates. It provides a single
 * link for the user to review and merge the intent layer changes.
 *
 * @param intentLayerPrNumber - The PR number of the intent layer PR
 * @param intentLayerPrUrl - The URL to the intent layer PR
 * @param updateCount - The number of intent updates included in the PR
 * @returns Comment body string with link to the intent layer PR
 */
export function generateIntentLayerLinkComment(
	intentLayerPrNumber: number,
	intentLayerPrUrl: string,
	updateCount: number,
): string {
	const updateWord = updateCount === 1 ? "update" : "updates";

	return `${INTENT_LAYER_LINK_MARKER}

## Intent Layer Updates Available

A separate pull request has been created with **${updateCount} intent layer ${updateWord}** based on the changes in this PR.

**[Review Intent Layer PR #${intentLayerPrNumber}](${intentLayerPrUrl})**

### What's included

- Updates to \`AGENTS.md\` and/or \`CLAUDE.md\` files
- Documentation changes for affected code areas

Please review and merge the intent layer PR to keep your documentation in sync with code changes.

---

*This comment was automatically generated by the Intent Layer GitHub Action.*`;
}

/**
 * Check if a comment contains the intent layer link marker.
 *
 * @param commentBody - The comment body to check
 * @returns True if the comment contains the intent layer link marker
 */
export function hasIntentLayerLinkMarker(commentBody: string): boolean {
	return commentBody.includes(INTENT_LAYER_LINK_MARKER);
}

/**
 * Result of posting an intent layer link comment.
 */
export interface IntentLayerLinkCommentResult {
	/** The ID of the created comment */
	commentId: number;
	/** The URL of the created comment */
	commentUrl: string;
...
/**
 * Post a single comment on the original PR linking to the intent layer PR.
 *
 * This function:
 * 1. Checks if an intent layer link comment already exists
 * 2. If it exists, updates it with the new PR info
 * 3. If not, creates a new comment
 *
 * @param client - GitHub client for API operations
 * @param originalPrNumber - The original PR number to post the comment on
 * @param intentLayerPrNumber - The intent layer PR number
 * @param intentLayerPrUrl - The URL to the intent layer PR
 * @param updateCount - The number of intent updates included in the PR
 * @returns Result containing the comment ID and URL
 */
export async function postIntentLayerLinkComment(
	client: GitHubClient,
	originalPrNumber: number,
	intentLayerPrNumber: number,
	intentLayerPrUrl: string,
	updateCount: number,
): Promise<IntentLayerLinkCommentResult> {
	// Generate the comment body
	const commentBody = generateIntentLayerLinkComment(
		intentLayerPrNumber,
		intentLayerPrUrl,
		updateCount,
	);

	// Check if an intent layer link comment already exists
	const existingComments = await client.getIssueComments(originalPrNumber);
	const existingLinkComment = existingComments.find(
		(comment) => comment.body && hasIntentLayerLinkMarker(comment.body),
	);

	if (existingLinkComment) {
		// Update the existing comment
		await client.updateComment(existingLinkComment.id, commentBody);
		return {
			commentId: existingLinkComment.id,
			commentUrl:
				existingLinkComment.html_url ??
				`https://github.com/pull/${originalPrNumber}#issuecomment-${existingLinkComment.id}`,
		};
	}
